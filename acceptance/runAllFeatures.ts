...
/**
 * Managed-mode orchestrator for the Gherkin acceptance suite.
 *
 * Mirrors runAllSuites.ts: the database + Astro server are started once, then
 * Cucumber.js is spawned (under Deno) with the shared SUT passed via env. The
 * Cucumber workers reuse that SUT instead of each starting their own.
 */
async function main() {
  ...
  try {
    ...
    const command = new Deno.Command(denoCmd, {
      args: [
        ...
        '--config',
        'acceptance/cucumber.json',
        '--parallel',
        Deno.env.get('DENO_JOBS') || '4',
        '--format',
        'junit:./acceptance-tests-report.xml',
      ],
      stdin: 'inherit',
      stdout: 'inherit',
      stderr: 'inherit',
